...
⚠️ **因此过滤逻辑只存在于 iOS（Phase 2），服务端没有这段代码，也没有对应的 vitest。** 过滤的单测在 Phase 2 用 Swift 写。
...
~~`POST /agent/prompt-classify`~~ —— **砍掉（2026-07-13 用户拍板）**：AI 建议 appliesTo 是凭空多一次
Claude 调用 + 等待，没有意义。新建默认「都行」（两个开关全勾，这本来就是设计稿 5a 脚注的语义），
用户要限定就手动改。5c 不再有琥珀提示条。（曾作为 Task 6 实现过又删除。）
