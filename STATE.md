...
## 进行中：Prompt Manager 重构 Phase 1 —— ref/fork 模型服务端（2026-07-13，代码完成待部署）
...
**PR = jianshuo.dev#22（branch `prompt-manager-server`，19+ commits，81 文件 / 985 测试全绿，
终审 READY）。待用户：合 PR + `cd agent && npx wrangler deploy`（自动权限门挡了自合自部署）。**
